...
class MockLLMStrategy(LLMStrategy):
    """Mock LLM strategy for enrichement"""

    # pylint: disable=too-few-public-methods

    def enrich(self, video_id: str, raw_text: str) -> str:
        """Must accept video id and raw text string and return response string"""
        if video_id == "FORCE_FAILURE":
            raise RuntimeError("Forced failure for testing")

        canned_payload = {
        "video_id": video_id,
        "cleaned_text": "This is sample cleaned text",
        "tech_terms": ["SELECT","VARIANT"],
        "book_names": ["Designing Data-Intensive Applications"],
        }
        return json.dumps(canned_payload)
